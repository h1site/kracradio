// supabase/functions/dropbox-upload-store/index.ts
// Upload store submission files (MP3 + cover image) to Dropbox for approval review
...
// Base folder for store approvals
const STORE_APPROVAL_FOLDER = '/store-approval';
...
serve(async (req) => {
  ...
  try {
    ...
    if (!body.store_product_id || !body.artist_name || !body.track_title) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields (store_product_id, artist_name, track_title)' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log('Processing upload for:', body.artist_name, '-', body.track_title);

    // Get Dropbox access token
    const accessToken = await getDropboxAccessToken();

    // Create artist folder path
    const artistFolder = `${STORE_APPROVAL_FOLDER}/${sanitizeFilename(body.artist_name)}`;
    await ensureFolderExists(accessToken, STORE_APPROVAL_FOLDER);
    ...
    // Update store_products with Dropbox URLs
    const updateData: any = {};
    if (results.audio_dropbox_url) {
      updateData.track_file_url = results.audio_dropbox_url;
    }
    if (results.cover_dropbox_url) {
      updateData.cover_image_url = results.cover_dropbox_url;
    }
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      if (updateError) {
        ...
        console.log('store_products updated with Dropbox URLs');
      }
    }
    ...
    console.error('Error in dropbox-upload-store:', error);
    return new Response(
